    ...
    <!-- Main content -->
      ...
      <div class="col-15">
           ...
           <div class="card card-success">
                ...
                <h3 class="card-title">Select Rent Book List for Return</h3>
              </div>
        <div class="card-body">
            <c:choose>
	        ...
	        <th>Return</th>
                ...
                <tbody>
              <c:forEach var="at" items="${rentReturnRegisterForm.rentReturnStudents}" varStatus="s">
			<tr>
				<td>${s.index+1}</td>
				<td>${at.stuRentId}</td>
				<td>${at.admin.adminName}</td>
				<td>${at.book.bookId}</td>
				<td>${at.book.bookName}</td>
				<td>${at.student.studentId}</td>
				<td>${at.student.studentName}</td>
				<td>${at.rentDate}</td>
				<td>${at.returnDate}</td>
				<td>${at.dueDate}</td>
				<td>${at.lateFeet}</td>
				<td><a href="returnBookSubmitPath?frmRentId=${at.stuRentId}" class="btn btn-block btn-outline-primary btn-sm">Return</a></td>
			</tr>		
		    </c:forEach>
                </tbody>
                <tfoot>
	        ...
	        <th>Return</th>
                ...
                <tbody>
              <c:forEach var="at" items="${rentReturnRegisterForm.rentReturnTeachers}" varStatus="s">
			<tr>
				<td>${s.index+1}</td>
				<td>${at.teaRentId}</td>
				<td>${at.admin.adminName}</td>
				<td>${at.book.bookId}</td>
				<td>${at.book.bookName}</td>
				<td>${at.teacher.teacherId}</td>
				<td>${at.teacher.teacherName}</td>
				<td>${at.rentDate}</td>
				<td>${at.returnDate}</td>
				<td>${at.dueDate}</td>
				<td>${at.lateFeet}</td>
				<td><a href="returnBookSubmitPath?frmRentId=${at.teaRentId}" class="btn btn-block btn-outline-primary btn-sm">Return</a></td>
			</tr>		
		</c:forEach>
                </tbody>
                <tfoot>
