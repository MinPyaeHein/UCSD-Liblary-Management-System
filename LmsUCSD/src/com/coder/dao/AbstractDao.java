...
public abstract class AbstractDao<PK extends Serializable,T> {
	...
	public List<T> criteriaQuerryGetObjectsByTwoPar(String sent,Date date){
		Session s=sessionFactory.openSession();
		Transaction tr = s.beginTransaction();
		CriteriaBuilder builder=s.getCriteriaBuilder();
		CriteriaQuery<T> querry=builder.createQuery(persistentClass);
		Root<T> root=querry.from(persistentClass);
		querry.select(root);
		querry.select(root).where(builder.and(builder.equal(root.get("sent"),sent)),(builder.equal(root.get("returnDate"),date)));
		Query<T> q=s.createQuery(querry);
		List<T> list=q.getResultList();
		tr.commit();
		s.close();
	...
	public List<T> criteriaQuerryGetObjectsByTwoPar(int sent,Date date){
		Session s=sessionFactory.openSession();
		Transaction tr = s.beginTransaction();
		CriteriaBuilder builder=s.getCriteriaBuilder();
		CriteriaQuery<T> querry=builder.createQuery(persistentClass);
