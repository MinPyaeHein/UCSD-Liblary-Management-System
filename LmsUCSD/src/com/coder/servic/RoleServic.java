...
import com.coder.form.RoleForm;
...
public class RoleServic {
	...
	public void prepareRoleRegister(RoleRegisterForm roleRegisterForm){
		RoleForm roleForm=roleRegisterForm.getRoleForm();
		if(roleForm!=null){
			int id=roleRegister(roleRegisterForm);
			}
		roleForm=new RoleForm();
	
	    List<Role> roles=this.roleDao.getRoles();
	    roleRegisterForm.setRoleForm(roleForm);
	    roleRegisterForm.setRoles(roles);
	}
	private int roleRegister(RoleRegisterForm roleRegisterForm){
		Role role=new Role();
		RoleForm roleForm=roleRegisterForm.getRoleForm();
		role.setRoleName(roleForm.getRoleName());
		int id=this.roleDao.saveRole(role);
		return id; 
	}
