...
import com.coder.form.BookEditionForm;
...
@Service
@Repository("bookEditionService")
public class BookEditionServic {
	@Autowired
	private BookEditionDao bookEditionDao;
	
	

	public void prepareBookEditionRegister(BookEditionRegisterFrom bookEditionRegisterFrom){
		BookEditionForm bookEditionForm=bookEditionRegisterFrom.getBookEditionForm();
		if(bookEditionForm!=null){
			int id=editionRegister(bookEditionRegisterFrom);
			}
		bookEditionForm=new BookEditionForm();
	    List<BookEdition> bookEditions=this.bookEditionDao.getBookEditions();
	    bookEditionRegisterFrom.setBookEditionForm(bookEditionForm);
	    bookEditionRegisterFrom.setBookEditions(bookEditions);
	}
	private int editionRegister(BookEditionRegisterFrom bookEditionRegisterFrom){
		BookEdition bookEdition=new BookEdition();
		BookEditionForm bookEditionFrom=bookEditionRegisterFrom.getBookEditionForm();
		bookEdition.setEditionName(bookEditionFrom.getEditionName());
		int id=this.bookEditionDao.saveBookEdition(bookEdition);
